...
function processPlyBuffer(inputBuffer: any): any {
    ...
    for (let prop of header
        ...
        .split("\n")
        .filter((k: any) => k.startsWith("property "))) {
    ...
    const attrs: any = new Proxy(
        {},
        {
            get(target, prop: any) {
                ...
                return (dataView as any)[types[prop]](
...
fs.readFile('./keila_joa_1.ply', (err: any, data: any) => {
    if (err) {
    ...
    fs.writeFile('./keila_joa_1.splat', Buffer.from(processedData), (err: any) => {
        if (err) {
